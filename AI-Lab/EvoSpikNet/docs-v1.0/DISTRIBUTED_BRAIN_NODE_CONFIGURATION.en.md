...
- Purpose: Provide quick reference for node configuration, model mapping, and hub setup in the distributed brain.
- Audience: Node deployment/model assignment implementers and ops.
- Read order: Overview → New architecture → Simulation types/node settings.
...
**Updated December 12, 2025**: Separated the classification managing motor cortex and sensor information to introduce a more efficient architecture.

### Sensor Hub (Sensor Hub)
- Integrates and manages all sensor inputs (visual, auditory, tactile)
- Responsible for preprocessing and integration of sensor data
- Provides integrated sensor data to PFC

### Motor Hub (Motor Hub)
- Integrates and manages all motor outputs (trajectory control, cerebellar coordination, PWM control)
- Converts PFC commands into actual motor control
- Manages coordination of multiple motor subsystems

### Advantages
- **Parallel Processing**: Enables simultaneous processing of sensor inputs
- **Specialization**: Each hub handles specialized functions
- **Extensibility**: Easy addition of new sensor/motor types
- **Efficiency**: Optimization of processing through separation of sensors and motors
