...
- **UIによるCRUD操作**: 「Data Generation」メニューの「Knowledge Base」ページは、知識ベースを直接管理するための強力なインターフェースです。
...
- **入力データ (UI → API → Simulation)**:
    ...
    3. APIは、受け取ったプロンプトデータを**インメモリストア**に一時的に保存します。
    4. Rank 0 (PFC) のシミュレーションプロセスは、APIエンドポイント `/api/distributed_brain/get_prompt` を定期的にポーリングし、新しいプロンプトを取得して処理を開始します。
...
- **出力データ (Simulation → API → UI)**:
    ...
    - **結果 (Result)**: Rank 0プロセスは、タスクの最終的なテキスト結果をAPIエンドポイント `/api/distributed_brain/result` にPOSTします。UIはこのエンドポイントをポーリングして「Query Response」エリアに結果を表示します。
